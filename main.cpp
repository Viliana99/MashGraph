...
#define REFRACT_GLASS  Material(BLACK,  vec(0.1, 1., 0.), 5000.0f, 0.f,   0.99f,  1.55f)
...
#define SHINE_GR Material(LIGHT_GREEN, vec(1., 1., 0.3), 50.f,  0.05f)
...
struct Scene {
    ...
    bool intersection_side(vec camera, vec dir, Cube& cube, vec& hit, float& min_dist, vec& normalb) {
        ...
        double t_near = numeric_limits<float>::min(), t_far = numeric_limits<float>::max();
        float t1, t2;
        ...
        if (t_near <= t_far && t_far >=0 && t_near < min_dist) {
            ...
            vec tmp = vec(0., 0., 0.);
            for (int i = 0; i < 3; i++) {
                if (abs(hit[i] - cube.vmin[i]) < eps2 * 0.01) 
                    tmp[i] = -1.;
                if (abs(hit[i] - cube.vmax[i]) < eps2 * 0.01)
                    tmp[i] = 1.;
            }
            normalb = tmp;
            return true;
    ...
    vec lighting_scene(const vec& camera, const vec& dir, int depth=0) {
        ...
        if ((depth < depth_scene) && intersection_ray(camera, dir, hit, normalb, material_pixel)) {
            ...
            if (material_pixel.refraction) {
                ...
                hit_shift = hit*normalb < 0 ? hit +  5*normalb : hit -  5*normalb; /////
        ...
        for (i = 0; i < WIDTH; i++) {
            for (j = 0; j < HEIGHT; j++) {
                ...
                if (!BMP) {
                    ...
                    bmp_frame[j][i][2] = (unsigned char) res.x; 
                    bmp_frame[j][i][1] = (unsigned char) res.y; 
                    bmp_frame[j][i][0] = (unsigned char) res.z; 
                } 
            }
        }
...
int main(int argc, char* argv []){
    ...
    vec setting_for_plane1 =  vec(1.,0.1, 0.5);
    Material m_for_plane1 = Material(BLACK,   setting_for_plane1, 50.f,  0.05f);
    Plane plane_chess =Plane(vec(4., 0., -1.), m_for_plane1);
    Scene my_scene(LIGHT_PINK, 7, plane_chess);

    my_scene.spheres.push_back(Sphere(vec(-3,    1,   -16), 2, GOLD));
    my_scene.spheres.push_back(Sphere(vec(0., -2, 2), 1.8, REFRACT_GLASS));
    my_scene.spheres.push_back(Sphere(vec(1.5, 0.5, -18), 3, MIRROR));
    my_scene.spheres.push_back(Sphere(vec(7., 5., -18), 3, SHINE_GR));
    my_scene.spheres.push_back(Sphere(vec(8., 8., -14), 2, GOLD));
    my_scene.spheres.push_back(Sphere(vec(8., 0., -12), 2, IRON));
    my_scene.spheres.push_back(Sphere(vec(7., -5., -18), 3, VELVET));
    my_scene.lamps.push_back(Light(vec(-30, 20,  20), 1.));
    ...
    my_scene.print_scene(camera, fileName.c_str(), flag_BMP, count_threads);
